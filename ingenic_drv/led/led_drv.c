...
//注意，此版本LED_GPRS_STATE和LED_SYS_STATE 调换

uint8_t LED_SYS_STATE  = 0;
uint8_t LED_GPRS_STATE = 2;
uint8_t LED_UART_STATE = 7;
...
void LED_CORL(void)
{
	...
	static uint8_t LED_ON_DELAY_TIME = 5;
	COUNT++;
//	printk("LED_GPRS_STATE = %d\r\n",LED_GPRS_STATE);
...
//				LED_NET_ON;
	...
	switch(LED_UART_STATE)
	{
		...
		case LED_WINK_SLOW:
			...
			break;
		case LED_ON_DELAY:
			if(LED_ON_DELAY_TIME--)
			{
				LED_UART_ON;
			}else
			{
				LED_ON_DELAY_TIME = 5;
				LED_UART_STATE = 7;//default close
			}
			break;
		default:
			LED_UART_OFF;
...
//		printk("led_buf = %s\r\n",led_buf);
		...
		{
			case '1':
//				LED_SYS_STATE = led_buf[1]-0x30;
				LED_GPRS_STATE = led_buf[1]-0x30;
				break;
			case '2':
//				LED_GPRS_STATE = led_buf[1]-0x30;
				LED_SYS_STATE = led_buf[1]-0x30;
